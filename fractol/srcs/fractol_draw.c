...
/*   Updated: 2020/07/05 17:56:52 by lcassaun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fr_zoom(t_fr *fr, int x, int y)
{
	...
	if (y > 0 && x > 0)
	{
		x_n = (x / fr->zoom) + fr->x1 + fr->shift_x_sum / fr->zoom;
		y_n = (y / fr->zoom) + fr->y1 + fr->shift_y_sum / fr->zoom;
		fr->zoom *= 1.35;
		fr->x1 = x_n - (x / fr->zoom) - fr->shift_x_sum / fr->zoom;
		fr->y1 = y_n - (y / fr->zoom) - fr->shift_y_sum / fr->zoom;
		fr_draw(fr);
	}
}
